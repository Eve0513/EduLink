...
export default async function FeedPage() {
  ...
  return <main className="min-h-screen bg-[#f3f6f7]">
    <StudentHeader name={me.full_name} avatarUrl={me.avatar_url} current="home" />
    <div className="mx-auto grid max-w-7xl gap-5 px-4 py-6 lg:grid-cols-[245px_minmax(0,1fr)_280px]">
      <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="-mx-5 -mt-5 h-16 rounded-t-2xl bg-[#0e5e6f]" /><ProfileAvatar src={me.avatar_url} name={me.full_name} className="-mt-7 h-14 w-14 border-4 border-white text-xl shadow-sm" /><h1 className="mt-3 font-bold">{me.full_name}</h1><p className="mt-1 text-sm text-slate-500">{me.headline || "Student EduLink"}</p><div className="mt-5 grid grid-cols-2 border-y border-slate-100 py-3 text-center text-xs"><div><b className="block text-base text-[#0e5e6f]">0</b>Vizualizări</div><div><b className="block text-base text-[#0e5e6f]">{following?.length ?? 0}</b>Urmăriri</div></div><a href="/dashboard/student/profile" className="mt-4 flex items-center gap-2 text-sm font-bold text-[#0e5e6f]"><UserRound className="h-4 w-4" />Vezi profilul</a></aside>
      <section className="space-y-5"><CreatePostCard name={me.full_name} />{posts.length ? posts.map((post) => <PostCard key={post.id} post={post} author={authorMap.get(post.creator_id)} />) : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">Feed-ul tău este pregătit. Urmărește organizații pentru conținut personalizat.</div>}</section>
      <aside className="space-y-5"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-bold">Recomandat pentru tine</h2><div className="mt-4 space-y-4">{(candidates ?? []).filter((company) => !followedCompanyIds.has(company.id)).slice(0, 3).map((company) => <div key={company.id} className="flex items-center gap-2"><div className="grid h-9 w-9 place-items-center rounded-full bg-[#e5f4f6] font-bold text-[#0e5e6f]">{company.name.slice(0, 1)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{company.name}</p><p className="truncate text-xs text-slate-500">{company.sector ?? "Companie parteneră"}</p></div><FollowButton targetType="company" targetId={company.id} initialIsFollowing={false} className="px-2" /></div>)}</div></section><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-bold">Evenimente viitoare</h2><div className="mt-4 space-y-4">{(events ?? []).map((event) => <div key={event.id} className="flex gap-3"><CalendarDays className="mt-0.5 h-5 w-5 text-[#168a9b]" /><div><p className="text-sm font-bold">{event.title ?? "Eveniment EduLink"}</p><p className="text-xs text-slate-500">{event.start_date ?? "Data urmează"} · {event.event_type}</p></div></div>)}</div></section></aside>
    </div>
  </main>;
}
