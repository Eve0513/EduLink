    ...
    <main className="min-h-screen overflow-hidden bg-[#f5f8f9]">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-6 px-4 py-7 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-6">
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="font-bold text-slate-900">Profil public și adresă URL</p><p className="mt-2 break-all text-sm text-slate-500">{portfolioUrl ?? "Linkul public va fi creat la finalizarea profilului."}</p>{portfolioUrl ? <Link href={portfolioUrl} target="_blank" className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-[#026a81] hover:underline"><ExternalLink className="h-4 w-4" />Previzualizează profilul public</Link> : null}</section>
...
function ProfileModal({ kind, selected, onClose }: { kind: ModalKind; selected: Selectable; onClose: () => void }) {
  ...
  function submit() {
    startTransition(async () => {
      ...
      else if (kind === "project") result = await saveProject({ id: project?.id, title: form.title, description: form.description, githubUrl: form.githubUrl, liveDemoUrl: form.liveDemoUrl, technologies: form.technologies.split(",").map((item) => item.trim()).filter(Boolean), imageUrl: project?.image_url ?? "" });
