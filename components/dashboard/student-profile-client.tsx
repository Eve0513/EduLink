...
import { isoToRomanianDate, normaliseRomanianDateInput } from "@/lib/romanian-date";
...
export function StudentProfileClient({
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f5f8f9]">
      ...
      {modal ? <ProfileModal kind={modal} selected={selected} onClose={() => setModal(null)} onSaved={() => router.refresh()} /> : null}
    </main>
  );
}
...
function ProfileModal({ kind, selected, onClose, onSaved }: { kind: ModalKind; selected: Selectable; onClose: () => void; onSaved: () => void }) {
  const [pending, startTransition] = useTransition();
...
function ModalFields({ kind, form, set }: { kind: ModalKind; form: Record<string, string>; set: (key: never, value: string) => void }) {
  ...
  if (kind === "certificate") return <><Field label="Nume *"><input value={form.title} onChange={(event) => setValue("title", event.target.value)} placeholder="Ex.: Certificare Microsoft" className={inputClass} /></Field><Field label="Organizație emitentă *"><input value={form.organization} onChange={(event) => setValue("organization", event.target.value)} placeholder="Ex.: Microsoft" className={inputClass} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Data emiterii"><input inputMode="numeric" placeholder="dd/mm/yyyy" value={form.issueDate} onChange={(event) => setValue("issueDate", normaliseRomanianDateInput(event.target.value))} className={inputClass} /></Field><Field label="Data expirării"><input inputMode="numeric" placeholder="dd/mm/yyyy" value={form.expiryDate} onChange={(event) => setValue("expiryDate", normaliseRomanianDateInput(event.target.value))} className={inputClass} /></Field></div><Field label="URL acreditare sau media"><input value={form.credentialUrl} onChange={(event) => setValue("credentialUrl", event.target.value)} className={inputClass} inputMode="url" /></Field><MediaUploadField label="certificatul (imagine sau PDF)" accept="image/png,image/jpeg,image/webp,application/pdf" onUploaded={(url) => setValue("credentialUrl", url)} /></>;
...
function formatDate(value: string | null) { return isoToRomanianDate(value) || "—"; }
