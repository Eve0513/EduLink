...
const checks = [
  { label: "8+ caractere", match: (value: string) => value.length >= 8 },
  { label: "O majusculă", match: (value: string) => /[A-Z]/.test(value) },
  { label: "O cifră", match: (value: string) => /\d/.test(value) },
  { label: "Un caracter special", match: (value: string) => /[^A-Za-z0-9]/.test(value) },
];
...
export function RegisterForm() {
  ...
  async function onSubmit(data: RegisterFormData) {
    ...
    if (!authData.user) {
      toast.error("Contul nu a putut fi creat. Încearcă din nou.");
      setLoading(false);
      return;
    }

    if (!authData.session) {
      toast.success("Ți-am trimis e-mailul de confirmare.");
    ...
    if (profileError) {
      toast.error(`Cont creat, dar profilul nu a putut fi pregătit: ${profileError.message}`);
      setLoading(false);
      return;
    }
