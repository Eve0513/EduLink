...
export function EduLinkMark({ light = false }: { light?: boolean }) {
  return <Link href="/" className="inline-flex items-center gap-2.5" aria-label="EduLink acasă"><span className={`grid h-11 w-11 place-items-center overflow-hidden rounded-xl ${light ? "bg-white/15" : "bg-[#e5f4f6]"}`}><Image src="/logo.png" alt="" width={96} height={96} className="h-[62px] w-[62px] max-w-none object-cover object-[center_24%]" /></span><span className={`bg-gradient-to-r ${light ? "from-white to-[#a6eff3]" : "from-[#003747] via-[#065465] to-[#026a81]"} bg-clip-text text-2xl font-extrabold tracking-tight text-transparent`}>EduLink</span></Link>;
}
