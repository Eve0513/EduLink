...
export function LoginForm() {
  ...
  async function onSubmit(data: LoginFormData) { setLoading(true); const { error } = await supabase.auth.signInWithPassword({ email: data.email, password: data.password }); if (error) { toast.error(error.message === "Invalid login credentials" ? "Email sau parolă incorectă" : error.message); setLoading(false); return; } toast.success("Autentificare reușită!"); router.refresh(); router.push("/dashboard"); }
