...
export const loginSchema = z.object({
  email: z.string().min(1, "Email-ul este obligatoriu").email("Introduceți un email valid"),
  password: z.string().min(1, "Parola este obligatorie").min(8, "Parola trebuie să aibă minimum 8 caractere"),
});

export const registerSchema = z.object({
  firstName: z.string().trim().min(2, "Prenumele trebuie să aibă minimum 2 caractere").max(50),
  lastName: z.string().trim().min(2, "Numele trebuie să aibă minimum 2 caractere").max(50),
  email: z.string().min(1, "Email-ul este obligatoriu").email("Introduceți un email valid"),
